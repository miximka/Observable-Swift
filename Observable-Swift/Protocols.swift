...
public protocol AnyEvent {
    ...
    mutating func add(_ handler : (ValueType) -> ()) -> EventSubscription<ValueType>
...
public func += <T : AnyObservable> (x: inout T, y: (ValueChange<T.ValueType>) -> ()) -> EventSubscription<ValueChange<T.ValueType>> {
    return x.afterChange += y
}

// observable += { (old, new) in ... }
public func += <T : AnyObservable> (x: inout T, y: (T.ValueType, T.ValueType) -> ()) -> EventSubscription<ValueChange<T.ValueType>> {
    return x.afterChange += y
}

// observable += { new in ... }
public func += <T : AnyObservable> (x: inout T, y: (T.ValueType) -> ()) -> EventSubscription<ValueChange<T.ValueType>> {
    return x.afterChange += y
}
...
public func <- <T : protocol<WritableObservable, UnownableObservable>> (x: inout T, y: T.ValueType) {
    x.value = y
}
